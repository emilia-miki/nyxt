...
If the `:setter-p' option is non-nil, then a dummy setf generic is defined too."
  (let* ((setter? (alex:assoc-value (mapcar #'uiop:ensure-list options) :setter-p))
         (normalized-options (if setter?
                                 (remove :setter-p options :key #'first)
                                 options)))
    `(progn
       (export-always ',name)
       (prog1
